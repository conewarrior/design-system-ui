...
# 파일이 components/ 내부이거나 page.tsx인지 확인
if [[ ! "$FILE_PATH" == *"components/"* ]] && [[ ! "$FILE_PATH" == *"page.tsx"* ]]; then
  exit 0
fi
...
# 3. 하드코딩 간격 탐지 (규칙 2.1) - 모든 px 값 금지
...
if [ -n "$HARDCODED_SPACING" ]; then
  echo -e "${RED}❌ [2.1 위반] 하드코딩 간격 사용 금지${NC}"
...
# 4. Anti-Pattern: border-left/right px 값 탐지 (Blockquote 등)
...
# 5. 불필요한 shadow 탐지 (규칙 2.8) - Modal, Dropdown, Toast가 아닌 경우
